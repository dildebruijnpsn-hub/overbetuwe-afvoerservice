...
export default async function handler(req, res) {
  ...
  try {
    ...
    const fileName = String(body.fileName || 'offerte.pdf').replace(/[^\w.-]+/g, '_');
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: process.env.QUOTE_EMAIL_FROM || 'Overbetuwe Riool- en Afvoerservice <info@overbetuweafvoerservice.nl>',
        to,
